...
Отвечай по-русски, кратко и по делу. Если вопрос короткий или неполный — всегда смотри на предыдущие сообщения в разговоре и отвечай в их контексте. Например, если спросили про противотуманки, а потом "какую ручку?" — отвечай про ручку переключателя освещения, а не про что-то другое. Не используй markdown-форматирование, звёздочки, решётки и другие спецсимволы — пиши обычным текстом. Если упоминаешь названия кнопок или надписей на панелях автомобиля — дублируй их на английском в скобках, например: кнопка запуска двигателя (Start/Stop).
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Привет! Я помогу с вопросами по VW Lavida 2022 (третья генерация) — от лампочек на панели до замены колеса. Спрашивайте!\n\n"
        "Если хотите уточнить ответ — задавайте следующий вопрос в течение 30 секунд и я пойму о чём речь."
    )
